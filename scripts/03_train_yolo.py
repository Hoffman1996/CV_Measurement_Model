...
def train_yolo_model():
    ...
    results = model.train(
        ...
        hsv_h=0.002,  # Reduce HSV hue augmentation
        hsv_s=0.2,  # Reduce HSV saturation augmentation
        hsv_v=0.15,  # Reduce HSV value augmentation
        erasing=0.05,  # Reduce random erasing
        mixup=0.0,  # Add slight mixup for better generalization
        copy_paste=0.0,  # Add copy-paste augmentation for object detection
        scale=0.1,  # Scale augmentation range
        translate=0.02,
        mosaic=0.3,  # Enable mosaic augmentation
        close_mosaic=20,  # Disable close mosaic to avoid overfitting
        degrees=2.0,  # Enable rotation augmentation
