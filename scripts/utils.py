...
def adjust_measurements_for_rotation(measurements, was_rotated):
    ...
    if 'width_cm' in measurements and 'height_cm' in measurements:
        ...
        measurements['width_cm'] = height_cm  # Original width becomes height after rotation
        measurements['height_cm'] = width_cm  # Original height becomes width after rotation
    
    return measurements

def detect_charuco_board(image, calib_data):
    """Detect ChArUco board with more permissive requirements"""
    ...
    if ids is not None and len(ids) >= 4:  # More permissive - reduced from 6
        ...
        if charuco_corners is not None and len(charuco_corners) >= 4:  # More permissive - reduced from 6
...
def project_points_to_board_plane(points_2d_px, calib, rvec, tvec):
    ...
    undist = cv2.undistortPoints(pts, K, dist)   # -> shape (N,1,2), normalized camera coords
...
def is_pose_reasonable(rvec, tvec):
    ...
    try:
        ...
        rotation_ok = rotation_magnitude < 2.5  # Less than ~143 degrees (more permissive)
...
def calculate_real_world_measurements_polygon(polygon_pixels, charuco_corners, charuco_ids, charuco_board, calib_data):
    ...
    if charuco_corners is None or len(charuco_corners) < 4:  # More permissive requirement
        return None
    ...
    if success:
        ...
        print(f"    DEBUG: tvec[2]={tvec[2].item():.3f}m, rotation={rotation:.2f}rad, inliers={inlier_count}/{len(charuco_corners)}")
        
        if not pose_ok:
            print(f"    WARNING: Unrealistic pose (dist={distance:.1f}m, rot={rotation:.2f}rad), skipping")
            return None
            
        # More permissive inlier requirement
        if inliers is not None and len(inliers) < max(3, len(charuco_corners) * 0.4):  # At least 40% or 3 points
            print(f"    WARNING: Too few inliers ({len(inliers)}/{len(charuco_corners)}), pose unreliable")
            return None
    else:
    ...
    polygon_corners_3d = project_points_to_board_plane(
        polygon_pixels.astype(np.float64),
        calib_data, 
        rvec, 
        tvec
    )
    ...
    try:
        ...
        if width_m > 15 or height_m > 15 or area_m2 > 200:  # Increased limits
            print(f"    WARNING: Measurements seem too large (w={width_m:.2f}m, h={height_m:.2f}m, a={area_m2:.2f}m²)")
            return None
            
        if width_m < 0.005 or height_m < 0.005:  # Smaller than 0.5cm
            print(f"    WARNING: Measurements seem too small (w={width_m:.4f}m, h={height_m:.4f}m)")
            return None
        
        # Debug output
        print(f"    DEBUG: area_m2={area_m2:.6f}, width_m={width_m:.3f}, height_m={height_m:.3f}, perimeter_m={perimeter_m:.3f}")
...
def calculate_real_world_measurements_bbox(bbox_pixels, charuco_corners, charuco_ids, charuco_board, calib_data):
    ...
    try:
        ...
        if width_m > 15 or height_m > 15:  # Increased from 10m
            print(f"    WARNING: BBox measurements too large (w={width_m:.2f}m, h={height_m:.2f}m)")
            return None
            
        if width_m < 0.005 or height_m < 0.005:  # Smaller than 0.5cm
            print(f"    WARNING: BBox measurements too small (w={width_m:.4f}m, h={height_m:.4f}m)")
            return None
...
def save_letterboxed_images_preview(input_dir, num_preview=3):
    ...
    for i, img_path in enumerate(images[:num_preview]):
        ...
        letterboxed, scale_ratio, padding = letterbox_yolo_style(image, (settings.YOLO_INPUT_SIZE, settings.YOLO_INPUT_SIZE), settings.LETTERBOX_COLOR)
        ...
        # Add text labels
        cv2.putText(comparison, f"Original: {original_shape[1]}x{original_shape[0]}", 
                   (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        cv2.putText(comparison, f"Letterboxed: {settings.YOLO_INPUT_SIZE}x{settings.YOLO_INPUT_SIZE}", 
                   (350, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        cv2.putText(comparison, f"Scale: {scale_ratio:.3f}", 
                   (350, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        cv2.putText(comparison, f"Padding: {padding[0]:.1f}x{padding[1]:.1f}", 
                   (350, 75), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        
        # Save comparison
        preview_path = os.path.join(preview_dir, f"preview_{i+1}_{os.path.basename(img_path)}")
...
def process_s20plus_image(image_path, model, calib_data, confidence_threshold=0.25):
    ...
    print(f"    Applied letterboxing: scale={scale_ratio:.3f}, padding=({padding[0]:.1f}, {padding[1]:.1f}), rotated={was_rotated}")
    
    # YOLO detection on letterboxed image
    results = model.predict(source=letterboxed_image, conf=confidence_threshold, verbose=False)
    result = results[0]
    
    # Detect ChArUco board on letterboxed image (same as calibration)
    charuco_corners, charuco_ids, charuco_board = detect_charuco_board(letterboxed_image, calib_data)
    ...
    for i, box in enumerate(result.boxes):
        ...
        if result.masks is not None and i < len(result.masks.data):
            ...
            if mask.shape != letterboxed_image.shape[:2]:
                mask = cv2.resize(mask, (letterboxed_image.shape[1], letterboxed_image.shape[0]))
            ...
            if contours:
                ...
                if charuco_corners is not None:
                    ...
                    if polygon_measurements:
                        ...
                        polygon_measurements = adjust_measurements_for_rotation(polygon_measurements, was_rotated)
        ...
        if detection['real_world_measurements'] is None and charuco_corners is not None:
            ...
            if bbox_measurements:
                # Adjust measurements for rotation to match original image orientation
                bbox_measurements = adjust_measurements_for_rotation(bbox_measurements, was_rotated)
